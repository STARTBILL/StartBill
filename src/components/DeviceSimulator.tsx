  ...
  return (
    <div className="w-full min-h-screen lg:h-screen bg-slate-50 flex flex-col lg:flex-row relative text-slate-800 font-sans overflow-hidden">
      ...
      <div className={`flex-1 bg-slate-50 flex flex-col overflow-y-auto relative text-slate-800 min-h-screen ${
        ...
        {/* Desktop Sticky Header Bar (SaaS Chrome) */}
        {showSaaSChrome && (
          <header className="hidden lg:flex h-16 bg-white border-b border-slate-200/90 px-8 items-center justify-between sticky top-0 z-20 shrink-0">
            {/* Left: Breadcrumbs & Page title */}
            <div className="flex items-center gap-3">
              <span className="text-xs font-semibold text-slate-400">StartBill Pro</span>
              <span className="text-slate-300">/</span>
              <h2 className="text-sm font-black text-slate-800 uppercase tracking-wider">
                {currentScreen === 'dashboard' ? 'Tableau de bord' :
                 currentScreen === 'invoices' ? 'Factures & Devis' :
                 currentScreen === 'expenses' ? 'Dépenses & Reçus' :
            ...
            {/* Right: Quick actions, Region selector, Notifications, Profile */}
            <div className="flex items-center gap-3 xl:gap-4">
              {/* Region Quick Selector Pill */}
              <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-bold">
                <button
                  type="button"
                  onClick={() => {
                    setCurrentRegion('canada');
                    triggerToast('Zone active : Canada 🇨🇦 (CAD, TPS/TVQ)');
                  }}
                  className={`px-2.5 py-1 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                    currentRegion === 'canada' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                  title="Canada (CAD $)"
                >
                  <span>🇨🇦</span>
                  <span className="hidden xl:inline">Canada</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setCurrentRegion('afrique');
                    triggerToast('Zone active : Afrique 🌍 (FCFA, Mobile Money)');
                  }}
                  className={`px-2.5 py-1 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                    currentRegion === 'afrique' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                  title="Afrique (FCFA)"
                >
                  <span>🌍</span>
                  <span className="hidden xl:inline">Afrique</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setCurrentRegion('haiti');
                    triggerToast('Zone active : Haïti 🇭🇹 (HTG / USD, MonCash)');
                  }}
                  className={`px-2.5 py-1 rounded-lg transition cursor-pointer flex items-center gap-1 ${
                    currentRegion === 'haiti' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                  title="Haïti (HTG)"
                >
                  <span>🇭🇹</span>
                  <span className="hidden xl:inline">Haïti</span>
                </button>
              </div>
              ...
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-xs transition cursor-pointer"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-amber-500/10 to-amber-600/10 border border-amber-300 text-amber-800 rounded-full text-[11px] font-black hover:bg-amber-100 transition cursor-pointer"
              >
                <Sparkles className="w-3 h-3 text-amber-600" />
                <span>{isPro ? 'PRO' : 'PASSER PRO'}</span>
              </button>
            </div>
          </header>
        )}
        ...
        {currentScreen === 'financial_health' && (
          <DetailedAnalysisPage
            ...
            currency={REGIONS[currentRegion]?.currency || 'CAD'}
          />
        )}
